...
function InviteContent() {
  ...
  const [state, setState] = useState<"loading" | "ready" | "error" | "declined">("loading");
  ...
  useEffect(() => {
    if (!token) { setState("error"); setErrorMsg("No invite token provided."); return; }
    fetch(`/api/learnhub/admin/invite-mentor/validate?token=${encodeURIComponent(token)}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.error) { setState("error"); setErrorMsg(data.error); }
        else { setInvite(data.invite); setState("ready"); }
      })
      .catch(() => { setState("error"); setErrorMsg("Failed to load invite. Please try again."); });
  }, [token]);

  const handleAccept = () => {
    window.location.href = `/api/learnhub/auth/google?callbackUrl=/learnhub/onboarding&invite_token=${encodeURIComponent(token)}`;
  };

  const handleDecline = async () => {
    await fetch(`/api/learnhub/admin/invite-mentor/validate?token=${encodeURIComponent(token)}`, {
      method: "DELETE",
    });
    setState("declined");
  };
          ...
          {state === "ready" && invite && (
            <>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                    {invite.programs.map((p) => (
                      <span key={p} style={{
                        ...
                        background: "rgba(91,108,255,0.15)", color: "#a0acff",
                      }}>{p}</span>
                    ))}
                  </div>
                )}
              ...
              {invite.personalMessage && (
                <p style={{ color: "#c8caf0", fontSize: 13, fontStyle: "italic", marginBottom: 20, textAlign: "center" }}>
                  "{invite.personalMessage}"
                </p>
              )}
...
export default function MentorInvitePage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#0d1025" }} />}>
      <InviteContent />
    </Suspense>
  );
}
