...
function InviteModal({ onClose, adminId }: { onClose: () => void; adminId: Id<"learnhub_users"> }) {
  ...
  if (sent) {
    return (
      <div style={{ textAlign: "center", padding: "32px 24px" }}>
        ...
        <h3 style={{ color: "#e8eaff", fontSize: 18, fontWeight: 700, marginBottom: 8 }}>Invite Sent!</h3>
        <p style={{ color: "#9ba3cc", fontSize: 14, marginBottom: 24 }}>
          An invitation has been sent to <strong style={{ color: "#e8eaff" }}>{form.invitedEmail}</strong>.
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <h3 style={{ color: "#e8eaff", fontFamily: "var(--font-sora)", fontSize: 18, fontWeight: 800, margin: 0 }}>
        Invite New Mentor
      </h3>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div>
          <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 5 }}>Full Name *</label>
        ...
        <div>
          <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 5 }}>Email Address *</label>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div>
          <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 5 }}>DICT Designation *</label>
        ...
        <div>
          <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 5 }}>Regional Office</label>
          <input style={FIELD_STYLE} value={form.regionalOffice} onChange={(e) => setForm((f) => ({ ...f, regionalOffice: e.target.value }))} />
        </div>
      </div>

      <div>
        <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 8 }}>Programs Handling</label>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: selected ? "rgba(91,108,255,0.2)" : "rgba(255,255,255,0.05)",
                  color: selected ? "#a0acff" : "#9ba3cc",
                  border: `1px solid ${selected ? "#5b6cff" : "rgba(255,255,255,0.1)"}`,
                }}
              >{p}</button>
            );
          })}
      ...
      <div>
        <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 8 }}>Expertise Areas</label>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {EXPERTISE_OPTIONS.map((ex) => {
            ...
            return (
              <button
                ...
                style={{
                  padding: "5px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
                  background: selected ? "rgba(34,211,160,0.15)" : "rgba(255,255,255,0.05)",
                  color: selected ? "#22d3a0" : "#9ba3cc",
                  border: `1px solid ${selected ? "#22d3a0" : "rgba(255,255,255,0.1)"}`,
                }}
              >{ex}</button>
            );
          })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 12 }}>
        <div>
          <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 5 }}>Max Mentees</label>
        ...
        <div>
          <label style={{ display: "block", color: "#9ba3cc", fontSize: 12, marginBottom: 5 }}>Personal Message (optional)</label>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
        <button type="button" onClick={onClose} style={{ padding: "10px 20px", borderRadius: 12, background: "rgba(255,255,255,0.06)", color: "#9ba3cc", border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer", fontWeight: 600 }}>
          Cancel
        </button>
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "24px 16px 60px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h1 style={{ color: "#e8eaff", fontFamily: "var(--font-sora)", fontSize: 20, fontWeight: 800, margin: 0 }}>
            Mentor Management
          </h1>
      ...
      <section style={{ marginBottom: 32 }}>
        <h2 style={{ color: "#9ba3cc", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>
          Pending Invites ({pendingInvites.length})
        </h2>
        {pendingInvites.length === 0 && (
          <div style={{ background: "#131626", borderRadius: 14, padding: "20px", border: "1px solid rgba(255,255,255,0.07)", textAlign: "center" }}>
            <p style={{ color: "#9ba3cc", fontSize: 14 }}>No pending invites</p>
          </div>
        )}
        {pendingInvites.map((inv) => {
          ...
          return (
            <div key={inv._id} style={{
              background: "#131626", borderRadius: 14, padding: "16px 20px", marginBottom: 8,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ color: "#e8eaff", fontWeight: 700, fontSize: 14, margin: 0 }}>{inv.invitedName}</p>
                <p style={{ color: "#9ba3cc", fontSize: 12, margin: "2px 0" }}>{inv.invitedEmail}</p>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                  {inv.programs.map((p) => (
                    <span key={p} style={TAG_STYLE("#7c8bff")}>{p}</span>
                  ))}
                  <span style={TAG_STYLE(daysLeft <= 2 ? "#ff5f6d" : "#9ba3cc")}>
                    {daysLeft}d left
                  </span>
                </div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, background: "rgba(255,255,255,0.06)", color: "#9ba3cc", border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer" }}>
                  Resend
                </button>
                <button
      ...
      <section style={{ marginBottom: 32 }}>
        ...
        {(activeMentors ?? []).length === 0 && (
          <div style={{ background: "#131626", borderRadius: 14, padding: "20px", border: "1px solid rgba(255,255,255,0.07)", textAlign: "center" }}>
            <p style={{ color: "#9ba3cc", fontSize: 14 }}>No active mentors yet</p>
          </div>
        )}
          ...
          <div key={mentor._id} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <p style={{ color: "#e8eaff", fontWeight: 700, fontSize: 14, margin: 0 }}>{mentor.name}</p>
                {mentor.mentorStatus === "verified" && (
                  <span style={TAG_STYLE("#22d3a0")}>✓ Verified</span>
                )}
                {mentor.mentorStatus === "pending_verification" && (
                  <span style={TAG_STYLE("#ff8c42")}>⏳ Pending</span>
                )}
              </div>
              <p style={{ color: "#9ba3cc", fontSize: 12, margin: "2px 0" }}>
                {mentor.designation ?? "DICT Trainer"} · {(mentor.currentMenteeCount ?? 0)} students
              </p>
        ...
        <section>
          ...
          {(pendingVerifications ?? []).map((ver) => (
            <div key={ver._id} style={{
              ...
              <div style={{ flex: 1 }}>
                <p style={{ color: "#e8eaff", fontWeight: 700, fontSize: 14, margin: 0 }}>Mentor ID: {ver.mentorId}</p>
                <p style={{ color: "#9ba3cc", fontSize: 12, margin: "2px 0" }}>
                  Submitted {new Date(ver.submittedAt).toLocaleDateString()}
                  {ver.dictEmployeeId && ` · Employee ID: ${ver.dictEmployeeId}`}
                </p>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
