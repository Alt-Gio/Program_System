...
// Recharts is split out of this page's initial chunk â€” the page shell
// (KPI tiles, headings) paints immediately, charts hydrate after.
const chartLoader = () => (
  <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--lh-text-2)", fontSize: 12 }}>
    Loading chartâ€¦
  </div>
);
...
function StatCard({ icon, label, value, sub }: { icon: string; label: string; value: string | number; sub?: string }) {
  return (
    <div style={{
      background: "var(--lh-card)", borderRadius: 16, padding: "20px 22px",
      border: "1px solid rgba(255,255,255,0.07)", flex: 1, minWidth: 0,
    }}>
      <p style={{ fontSize: 22, marginBottom: 8 }}>{icon}</p>
      <p style={{ color: "var(--lh-text-2)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{label}</p>
      <p style={{ color: "var(--lh-text)", fontSize: 28, fontWeight: 800, fontFamily: "var(--font-sora)", lineHeight: 1 }}>{value}</p>
      {sub && <p style={{ color: "#22d3a0", fontSize: 11, marginTop: 4 }}>{sub}</p>}
    </div>
  );
}

// â”€â”€ Province avatar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function ProvinceAvatar({ name, count }: { name: string; count: number }) {
  ...
  return (
    <div title={name} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
      ...
      <span style={{ color: "var(--lh-text-2)", fontSize: 10 }}>{count}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ color: "var(--lh-text)", fontSize: 13, fontWeight: 700 }}>{count.toLocaleString()} ({pct}%)</span>
      </div>
      <div style={{ height: 8, borderRadius: 4, background: "var(--lh-border)", overflow: "hidden" }}>
    ...
    <>
      ...
      <div className="report-content" style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px 60px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
          <div>
            <p style={{ color: "#5b6cff", fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>
              ILCDB Â· Mentor Report
            </p>
            <h1 style={{ color: "var(--lh-text)", fontFamily: "var(--font-sora)", fontSize: 22, fontWeight: 800, margin: 0 }}>
              {year} Quarterly Accomplishment
            </h1>
            <p style={{ color: "var(--lh-text-2)", fontSize: 13, marginTop: 4 }}>
              Detailed analysis of digital literacy initiatives â€” {quarter} {year}
            </p>
          </div>
          <div className="report-actions" style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            {/* Quarter selector */}
            <div style={{ display: "flex", gap: 1, background: "var(--lh-card-3)", borderRadius: 12, padding: 4 }}>
              {QUARTERS.map((q) => (
                <button
                  key={q}
                  ...
                  style={{
                    ...
                    color: quarter === q ? "#fff" : "var(--lh-text-2)",
                  }}
                >{q}</button>
              ))}
            </div>
            ...
            <select
              ...
              style={{
                background: "var(--lh-border)", border: "1px solid rgba(255,255,255,0.1)",
                borderRadius: 10, color: "var(--lh-text)", padding: "6px 12px", fontSize: 13, cursor: "pointer",
              }}
            >
        ...
        <div className="report-card" style={{
          background: "var(--lh-card)", borderRadius: 16, padding: "20px 24px", marginBottom: 16,
          ...
          <p style={{ color: "var(--lh-text-2)", fontSize: 11, marginTop: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>
            VERIFIED OPERATIONS TEAM Â· Bicol Region â€” {QUARTERS.indexOf(quarter) === 0 ? "January" : QUARTERS.indexOf(quarter) === 1 ? "April" : QUARTERS.indexOf(quarter) === 2 ? "July" : "October"} to {QUARTERS.indexOf(quarter) === 0 ? "March" : QUARTERS.indexOf(quarter) === 1 ? "June" : QUARTERS.indexOf(quarter) === 2 ? "September" : "December"} {year}
          </p>
        </div>

        {/* Stat Cards */}
        <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
          <StatCard icon="ðŸ“‹" label="Total Trainings" value={isLoading ? "â€”" : (reportData?.totalTrainings ?? 0)} sub="Sessions conducted" />
          <StatCard icon="ðŸ‘¥" label="Beneficiaries" value={isLoading ? "â€”" : totalBeneficiaries.toLocaleString()} sub="Successful completions" />
          <StatCard icon="ðŸ“" label="Province Coverage" value={isLoading ? "â€”" : `${reportData?.provinceCoverage ?? 0}%`} sub={`${reportData?.coveredProvinces?.length ?? 0} of 6 provinces`} />
          {specialCampaign && (
            <StatCard icon="âš¡" label={`${specialCampaign.tag} Impact`} value={specialCampaign.studentCount.toLocaleString()} sub={`${specialCampaign.eventCount} events`} />
          )}
        </div>

        {/* Charts Row 1: Modality + Province */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginBottom: 16 }}>
          <div className="report-card" style={{ background: "var(--lh-card)", borderRadius: 16, padding: "20px", border: "1px solid rgba(255,255,255,0.07)" }}>
            <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 16 }}>Training Modality</p>
            {modalityData.length > 0 ? (
              <ModalityPieChart data={modalityData} />
            ) : (
              <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <p style={{ color: "var(--lh-text-2)", fontSize: 13 }}>No data for {quarter} {year}</p>
              </div>
            )}
          </div>

          <div className="report-card" style={{ background: "var(--lh-card)", borderRadius: 16, padding: "20px", border: "1px solid rgba(255,255,255,0.07)" }}>
            <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 16 }}>Beneficiaries by Province</p>
        ...
        {specialCampaign && (
          <div className="report-card" style={{
            ...
            <div>
              ...
              <p style={{ color: "var(--lh-text)", fontSize: 26, fontWeight: 800, marginBottom: 4 }}>{specialCampaign.studentCount.toLocaleString()}</p>
              <p style={{ color: "var(--lh-text-2)", fontSize: 12 }}>Participants</p>
            ...
            <div>
              <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>
        ...
        {!specialCampaign && (reportData?.trainingModules ?? []).length > 0 && (
          <div className="report-card" style={{
            ...
            <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>Training Modules</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginBottom: 16 }}>
          <div className="report-card" style={{ background: "var(--lh-card)", borderRadius: 16, padding: "20px", border: "1px solid rgba(255,255,255,0.07)" }}>
            <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 16 }}>Beneficiary Sector</p>
            {sectorData.length > 0 ? (
              <SectorPieChart data={sectorData} />
            ) : (
              <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <p style={{ color: "var(--lh-text-2)", fontSize: 13 }}>No sector data yet</p>
              </div>
            )}
          </div>

          <div className="report-card" style={{ background: "var(--lh-card)", borderRadius: 16, padding: "20px", border: "1px solid rgba(255,255,255,0.07)" }}>
            <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 16 }}>Key Category Performance</p>
            {categoryData.length > 0 ? (
              <CategoryBarChart data={categoryData} />
            ) : (
              <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <p style={{ color: "var(--lh-text-2)", fontSize: 13 }}>No category data yet</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="report-card" style={{
          ...
          <div>
            <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 16 }}>Gender Demographics</p>
            <GenderBar label="Female" count={genderBreakdown.female} total={totalGendered} color="#f472b6" />
            <GenderBar label="Male" count={genderBreakdown.male} total={totalGendered} color="#5b6cff" />
            {genderBreakdown.prefer_not_to_say > 0 && (
              <GenderBar label="Prefer not to say" count={genderBreakdown.prefer_not_to_say} total={totalGendered} color="var(--lh-text-2)" />
            )}
          </div>
          ...
          {vulnerableTotal > 0 && (
            <div>
              <p style={{ color: "var(--lh-text-2)", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>
                ...
                {Object.entries(reportData?.vulnerableGroups ?? {})
                  ...
                  .map(([k, v]) => (
                    <div key={k} style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      <span style={{ color: "var(--lh-text)", fontWeight: 700, fontSize: 12 }}>{(v as number).toLocaleString()}</span>
                    </div>
                  ))}
              </div>
            </div>
