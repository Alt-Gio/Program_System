...
const BADGES: Record<number, string> = { 1: "ðŸ¥‡", 2: "ðŸ¥ˆ", 3: "ðŸ¥‰" };
...
function getLevel(xp: number) {
  if (xp < 500) return { level: 1, label: "Seedling", color: "var(--lh-text-2)" };
...
export default function LeaderboardPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{ color: "var(--lh-text)", fontFamily: "var(--font-sora)" }}>Leaderboard</h1>
        <p className="text-sm mt-0.5" style={{ color: "var(--lh-text-2)" }}>Top learners by XP this month</p>
      </div>
          ...
          {[users[1], users[0], users[2]].map((u, i) => {
            ...
            return (
              <div key={u._id} className="flex flex-col items-center gap-2" style={{ width: 96 }}>
                ...
                <p className="text-xs font-semibold text-center truncate w-full" style={{ color: "var(--lh-text)", fontFamily: "var(--font-sora)" }}>{u.name.split(" ")[0]}</p>
      ...
      <div className="flex flex-col gap-2">
        {users === undefined && (
          Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background: "var(--lh-card)" }} />
          ))
        )}
        {users?.map((u) => {
          const level = getLevel(u.xpPoints);
          ...
          return (
            <div key={u._id} className="flex items-center gap-3 rounded-xl px-4 py-3" style={{ background: isTop3 ? "rgba(91,108,255,0.06)" : "var(--lh-card)", border: isTop3 ? "1px solid rgba(91,108,255,0.15)" : "1px solid rgba(255,255,255,0.05)" }}>
              <span className="w-7 text-center text-sm font-bold shrink-0" style={{ color: isTop3 ? "#ffd700" : "var(--lh-text-3)", fontFamily: "var(--font-sora)" }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-semibold truncate" style={{ color: "var(--lh-text)", fontFamily: "var(--font-sora)" }}>{u.name}</p>
                ...
                <p className="text-xs mt-0.5" style={{ color: "var(--lh-text-2)" }}>ðŸ”¥ {u.currentStreak}-day streak Â· {u.badges.length} badge{u.badges.length !== 1 ? "s" : ""}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-sm font-bold" style={{ color: "#5b6cff", fontFamily: "var(--font-sora)" }}>{u.xpPoints.toLocaleString()}</p>
                <p className="text-[10px]" style={{ color: "var(--lh-text-3)" }}>XP</p>
              </div>
            </div>
          );
        })}
        {users?.length === 0 && (
          <p className="text-center text-sm py-12" style={{ color: "var(--lh-text-3)" }}>No participants yet. Be the first on the board!</p>
        )}
      </div>
    </div>
  );
