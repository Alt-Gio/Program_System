    ...
    <>
      ...
      {!open && (
        ...
        >
          ...
          <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center" style={{ background: "#22d3a0", color: "var(--lh-input-bg)" }}>AI</span>
        </button>
      )}

      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-80 sm:w-96 rounded-2xl overflow-hidden flex flex-col shadow-2xl" style={{ height: 480, background: "var(--lh-card)", border: "1px solid rgba(91,108,255,0.25)" }}>
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: "linear-gradient(135deg, rgba(91,108,255,0.15) 0%, rgba(34,211,160,0.1) 100%)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold" style={{ color: "var(--lh-text)", fontFamily: "var(--font-sora)" }}>AI Study Assistant</p>
              <p className="text-[10px]" style={{ color: "var(--lh-text-2)" }}>Powered by Groq · llama-3.1-8b</p>
            </div>
            <button onClick={() => setOpen(false)} className="p-1 rounded-lg opacity-60 hover:opacity-100" style={{ color: "var(--lh-text-2)" }}>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                ...
                <p className="text-sm font-medium" style={{ color: "var(--lh-text)", fontFamily: "var(--font-sora)" }}>Ask me anything!</p>
                <p className="text-xs" style={{ color: "var(--lh-text-2)" }}>I&apos;m here to help you study, understand concepts, or prepare for your ILCDB program assessments.</p>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[85%] rounded-xl px-3 py-2 text-sm" style={{ background: msg.role === "user" ? "#5b6cff" : "var(--lh-card-2)", color: msg.role === "user" ? "#fff" : "var(--lh-text)" }}>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="rounded-xl px-4 py-3" style={{ background: "var(--lh-card-2)" }}>
          ...
          <div className="p-3 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex items-end gap-2">
              <textarea
                ...
                style={{ background: "var(--lh-input-bg)", border: "1px solid rgba(255,255,255,0.08)", color: "var(--lh-text)", maxHeight: 100 }}
