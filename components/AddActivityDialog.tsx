...
export function AddActivityDialog({
  ...
  // Venue map picker — Venue (text) and Coordinates (lat/lng) are kept
  // strictly separate. The picker only sets coords + suggests a place
  // name; the user can click a button to copy it into Venue if empty.
  ...
  // Image upload — the file is uploaded to Convex storage for an instant
  // preview, AND (when a title is set) pushed to the project's Drive
  // folder so the Google Sheet gets a real Drive link (not a short-lived
  // Convex signed URL). driveImageUrl is the authoritative "permanent"
  // URL used for rawPhotosLink + sheet imageUrl when available.
  const fileInputRef                         = useRef<HTMLInputElement>(null);
  const [uploadedStorageId, setUploadedStorageId] = useState<string | null>(null);
  const [imageUploading,    setImageUploading]    = useState(false);
  const [pendingImageFile,  setPendingImageFile]  = useState<File | null>(null);
  const [driveImageUrl,     setDriveImageUrl]     = useState<string | null>(null);
  ...
  // Prefer the Drive URL if we have it — it's permanent and embeds
  // cleanly into the Google Sheet via =IMAGE().
  const effectiveImageUrl = driveImageUrl ?? uploadedImageUrl ?? null;
  ...
  async function handleImageUpload(file: File) {
    ...
    try {
      // 1. Convex storage for instant preview
    ...
    // 2. If a title exists, also push to Drive in the background so the
    // sheet gets a permanent Drive URL. Otherwise defer until Save.
    if (title.trim()) void pushImageToDrive(file, title.trim());
  }

  // Uploads a File to the project's Drive folder via /api/upload-image.
  // Returns the resulting Drive URL on success (also stored in state).
  async function pushImageToDrive(file: File, activityTitle: string): Promise<string | null> {
    if (!file || !activityTitle) return null;
    setDriveUploading(true);
    try {
      const fd = new FormData();
      fd.append("file", file);
      fd.append("activityTitle", activityTitle);
      fd.append("projectCode", projectDef.code);
      const r = await fetch("/api/upload-image", { method: "POST", body: fd });
      const j = await r.json();
      if (!r.ok || !j.url) {
        // Drive is optional — warn, don't block. The sheet just won't get
        // a permanent URL.
        console.warn("[drive-upload]", j);
        toast.warning(`Drive upload skipped: ${j.error ?? "unknown"} — sheet will use a preview URL instead.`, { duration: 6000 });
        return null;
      }
      setDriveImageUrl(j.url);
      toast.success(`Saved to Drive: "${activityTitle}" / Raw Photos`, { icon: "📁" });
      return j.url as string;
    } catch (e) {
      console.warn("[drive-upload]", e);
      toast.warning("Drive upload unavailable — the image is still saved in the app.", { duration: 5000 });
      return null;
    } finally {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      // 0. If we still have a raw file but no Drive URL, push it now so
      // the sheet gets a permanent link instead of a Convex signed URL.
      let finalDriveUrl = driveImageUrl;
      if (!finalDriveUrl && pendingImageFile) {
        finalDriveUrl = await pushImageToDrive(pendingImageFile, title.trim());
      }
      const imageUrl = finalDriveUrl ?? uploadedImageUrl ?? undefined;
      ...
      const activityId = await createActivity({
        ...
        venue: venue.trim(),
        coordinates: venueCoords ?? undefined,
        ...
        rawPhotosLink: photosLink || finalDriveUrl || uploadedImageUrl || undefined,
          ...
          const res = await fetch("/api/activity-to-sheet", {
            ...
            body: JSON.stringify({
              ...
              activity: {
                ...
                afterActivityReport: afterReport || undefined,
                fbPostingLink: fbLink || undefined,
                rawPhotosLink: photosLink || finalDriveUrl || undefined,
                imageUrl: finalDriveUrl ?? uploadedImageUrl ?? undefined,
                testimonialsLink: testimLink || undefined,
                extraFields: extra,
                participants: pax,
              },
  ...
  function handleMapConfirm(loc: PickedLocation) {
    ...
    setPickedPlaceName(loc.placeName);
    toast.success("Coordinates set. Venue field is separate — edit it above as needed.", { icon: "📍" });
  }
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. Legazpi City Hall, DICT Regional Office…"
                    className="h-9 text-sm"
                  />
                  {pickedPlaceName && !venue && (
                    <button type="button"
                      onClick={() => setVenue(pickedPlaceName)}
                      className="text-[11px] text-purple-600 hover:underline mt-1">
                      Use picked location name: <span className="italic">{pickedPlaceName.length > 60 ? pickedPlaceName.slice(0, 60) + "…" : pickedPlaceName}</span>
                    </button>
                  )}
                </div>
                ...
                <div>
                  <Label className="text-xs font-semibold text-gray-600 mb-1 block">
                    Coordinates <span className="text-gray-400 font-normal normal-case">(for Mapbox)</span>
                  </Label>
                  <div className="flex gap-2">
                    <div className={`flex-1 h-9 px-3 rounded-lg border text-sm flex items-center gap-2 font-mono ${
                      venueCoords ? "bg-white text-gray-800 border-purple-200" : "bg-gray-50 text-gray-400 border-gray-200"
                    }`}>
                      <MapPin className={`w-3.5 h-3.5 shrink-0 ${venueCoords ? "text-purple-600" : "text-gray-300"}`} />
                      {venueCoords ? (
                        <span className="truncate">
                          {venueCoords.lat.toFixed(5)}, {venueCoords.lng.toFixed(5)}
                        </span>
                      ) : (
                        <span className="italic">No pin dropped — click Pick on Map</span>
                      )}
                    </div>
                    <Button type="button" size="sm" variant="outline"
                      ...
                      {venueCoords ? "Re-pick" : "Pick on Map"}
                    </Button>
                    {venueCoords && (
                      <Button type="button" size="sm" variant="ghost"
                        onClick={() => { setVenueCoords(null); setPickedPlaceName(""); }}
                        className="h-9 px-2 shrink-0 text-gray-400 hover:text-red-600">
                        <X className="w-3.5 h-3.5" />
                      </Button>
                    )}
                  </div>
                  {pickedPlaceName && venueCoords && (
                    <p className="text-[10px] text-gray-400 mt-1 truncate">
                      Near: <span className="italic">{pickedPlaceName}</span>
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                {effectiveImageUrl ? (
                  <div className="relative rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
                    ...
                    <img src={effectiveImageUrl} alt="Activity" className="w-full max-h-40 object-cover" />
                    <button type="button"
                      onClick={() => { setUploadedStorageId(null); setPendingImageFile(null); setDriveImageUrl(null); }}
                    ...
                    <div className="px-3 py-2 text-[11px] text-gray-500 flex items-center gap-1.5">
                      {driveUploading
                        ? <><Loader2 className="w-3 h-3 animate-spin" /> Uploading to project Drive folder…</>
                        : driveImageUrl
                        ? <><CheckCircle2 className="w-3 h-3 text-green-600" /> Saved to Drive → <em className="ml-0.5">{projectDef.shortName} / {title || "(title)"} / Raw Photos</em></>
                        : title.trim()
                        ? <><CheckCircle2 className="w-3 h-3 text-green-600" /> Preview ready — will save to Drive on Save</>
                        : <>Preview ready — enter a title to enable Drive upload.</>}
                    </div>
                  </div>
                ) : (
