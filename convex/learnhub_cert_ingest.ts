/**
 * Gmail-driven certificate ingest (Convex side).
 *
 * `pollGmail` runs on a 15-min cron. It:
 *   1. Reads the active `learnhub_cert_email_config` row.
 *   2. Decrypts the admin user's stored Gmail tokens (refreshes if near
 *      expiry, persists the new access token back).
 *   3. Hits Gmail REST directly for messages from the trusted sender
 *      that carry image/PDF attachments.
 *   4. For each new message, downloads attachments to Convex `_storage`
 *      and writes one `learnhub_certificates` row per recipient.
 *
 * We intentionally call Gmail REST via `fetch` and inline the crypto here
 * rather than dynamic-importing the Next.js `lib/learnhub/*` helpers — the
 * Convex tsconfig doesn't share the `@/*` alias, so re-using those modules
 * forced a circular typecheck. One file, no aliases, no extra packages.
 */
...
// ── Crypto (inlined from lib/learnhub/token-crypto) ─────────────────────
...
// ── Internal helpers (callable only by actions) ─────────────────────────
...
// Per-message worker. Extracted so the action body stays readable. The
// ctx type uses Convex's standard GenericActionCtx<DataModel> alias.
async function ingestOneMessage(
  ...
  // Walk the MIME tree for image/PDF parts that carry an attachmentId.
  ...
  for (const att of attachments) {
    ...
    // Re-wrap into a fresh ArrayBuffer to satisfy Blob's BlobPart type —
    // Node's Buffer-backed Uint8Array uses ArrayBufferLike which TS
...
// Public wrapper so an admin can trigger a manual poll from the UI.
export const triggerManualPoll = action({
  ...
  handler: async (ctx, args): Promise<IngestSummary> => {
    ...
    return await ctx.runAction(internal.learnhub_cert_ingest.pollGmail, {});
  },
});

// Provide a small `getStorageUrl` query so the certificates page can
// render the ingested attachment without exposing internal storage IDs
// directly. Mirrors the helper in learnhub_posts.ts.
